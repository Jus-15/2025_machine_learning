...
Original file is located at
    https://colab.research.google.com/drive/18HfJKTpwshsQOZnMkrSkIkwkJxX0butb
"""
...
def runge(x):
    # f(x) = 1 / (1 + 25 x^2)
    f  = 1.0 / (1.0 + 25.0 * x**2)
    df = (-50.0 * x) / ((1.0 + 25.0 * x**2)**2)
    return np.hstack([f, df])
rng1 = np.random.default_rng()  # 固定隨機種子
...
W4, b4 = Weight_Initialization(2, H2, rng2)  # 第三層: H2 -> 1
...
err = np.abs(y_pred_test - y_test.T)

max_err_f  = np.max(err[0, :])   # f(x) 的最大誤差
max_err_df = np.max(err[1, :])   # f'(x) 的最大誤差


print(f"Train MSE: {MSE_train:.6f}")
print(f"Val  MSE: {MSE_val:.6f}")
print(f"Test  MSE: {MSE_test:.6f}")
print(f"Test Max Error f(x):  {max_err_f:.6f}")
print(f"Test Max Error f'(x): {max_err_df:.6f}")
...
axs[0].plot(xx, yy_true[:, 0], label="True f(x)")
axs[0].plot(xx, yy_pred[:, 0], label="NN f(x)")
axs[0].scatter(x_train, y_train[:, 0], s=8, alpha=0.3, label="Train f(x)")
axs[0].set_title("Function value")
axs[0].legend(); axs[0].grid(True, alpha=0.2)

axs[1].plot(xx, yy_true[:, 1], label="True f'(x)")
axs[1].plot(xx, yy_pred[:, 1], label="NN f'(x)")
axs[1].scatter(x_train, y_train[:, 1], s=8, alpha=0.3, label="Train f'(x)")
axs[1].set_title("Derivative")
axs[1].legend(); axs[1].grid(True, alpha=0.2)
